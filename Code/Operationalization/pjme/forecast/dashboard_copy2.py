...
logging.info("imprimindo info e head de data_split ")
data_split.columns =['data_medicao','medicao_mw']


data_split['data_medicao'] = data_split['data_medicao'].astype('datetime64[ns]')
...
print(measurements_forecast_data.info())
print(measurements_forecast_data_all.info())



measurements_forecast_data['Year'] = measurements_forecast_data['fields.date'].dt.year
measurements_forecast_data['Month'] = measurements_forecast_data['fields.date'].dt.month
measurements_forecast_data['Day'] = measurements_forecast_data['fields.date'].dt.day

measurements_forecast_data_all['Day'] = measurements_forecast_data_all['fields.date'].dt.day
measurements_forecast_data_all['Month'] = measurements_forecast_data_all['fields.date'].dt.month
measurements_forecast_data_all['data_sem_hora'] = pd.to_datetime(measurements_forecast_data_all['fields.date']).dt.normalize()


function_dict = {"medicao_mw": "sum", "fields.PJME_MW": "sum", "fields.error": "sum", "fields.percentage_error": "mean"}

function_dict_measurements_forecast_data_all = {"fields.PJME_MW": "sum", "fields.error": "sum", "fields.percentage_error": "mean"}
...
day_data_all = measurements_forecast_data_all.groupby('data_sem_hora', as_index=False)
...
def update_dash():
    ...
    plot = go.Figure(data=[go.Scatter(
        x=pd.Series(measurements_forecast_data_all["fields.date"]), y=pd.Series(measurements_forecast_data_all["fields.percentage_error"]), 
        mode = 'markers', 
        marker=dict( 
...
def update_graph_raw(ndays_plot):    
    ydata = measurements_forecast_data['medicao_mw']
    xdata = measurements_forecast_data['data_medicao']
    xdata_forecast = measurements_forecast_data['fields.date']
    ydata_forecast = measurements_forecast_data['fields.PJME_MW']
...
def update_graph_forecast(ndays_plot): 
    xdata_forecast_all = measurements_forecast_data_all['fields.date']
    ydata_forecast_all = measurements_forecast_data_all['fields.percentage_error']
...
def update_graph_forecast(ndays_plot): 
    ...
    print(corr)
    print(medicao_historica_periodo_treinamento.shape)
